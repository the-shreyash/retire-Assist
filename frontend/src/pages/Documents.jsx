...
import { FolderOpen, Upload, Download, Trash2, Eye, Shield, Lock, FileText, Image, ScanLine, CheckCircle, Loader } from 'lucide-react';

const fadeUp = { hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } };
const stagger = { visible: { transition: { staggerChildren: 0.08 } } };

const docTypeEmoji = { 'aadhaar': '🪪', 'pan': '💳', 'pension_id': '📋', 'insurance': '🛡️', 'bank_statement': '🏦', 'life_certificate': '📜' };
...
export default function Documents() {
    ...
    const fetchDocs = () => { API.get('/documents').then(r => setDocuments(r.data || [])).catch(() => { }).finally(() => setLoading(false)); };
    useEffect(fetchDocs, []);

    const handleUpload = async (e) => {
        e.preventDefault();
        if (!file) return;
        setUploading(true); setScanResult(null); setScanning(true);
        ...
        try {
            ...
            fetchDocs(); setFile(null);
        } catch { } finally { setUploading(false); }
    };

    const handleDelete = async (id) => { try { await API.delete(`/documents/${id}`); fetchDocs(); } catch { } };
    ...
    return (
        <motion.div initial="hidden" animate="visible" variants={stagger}>
            ...
            <motion.div variants={fadeUp} className="card mb-5">
                ...
                <form onSubmit={handleUpload} className="flex flex-col sm:flex-row items-end gap-3">
                    ...
                    <div className="flex-1 w-full">
                        ...
                        <div className="border-2 border-dashed border-border rounded-lg p-3 text-center hover:border-primary-light transition-all cursor-pointer" onClick={() => document.getElementById('doc-upload')?.click()}>
                            <input type="file" id="doc-upload" className="hidden" onChange={e => setFile(e.target.files?.[0])} accept=".pdf,.jpg,.jpeg,.png" />
                            <p className="text-sm font-medium text-text">{file ? file.name : 'Click to select file'}</p>
                        </div>
                    </div>
                    <button type="submit" disabled={uploading || !file} className="btn-primary !px-6 disabled:opacity-50 w-full sm:w-auto">
            ...
            <motion.div variants={stagger} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {loading ? Array(3).fill(0).map((_, i) => <div key={i} className="card animate-pulse h-40 bg-bg" />) :
                    documents.length === 0 ? (
                        ...
                        <motion.div key={doc._id} variants={fadeUp} className="card group hover:border-primary/20">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <span className={`badge ${doc.status === 'verified' ? 'badge-success' : doc.status === 'processing' ? 'badge-warning' : 'badge-info'}`}>{doc.status}</span>
                            </div>
                            <h3 className="font-bold text-sm text-text-dark mb-0.5">{doc.type?.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</h3>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => handleDelete(doc._id)} className="btn-ghost !px-2 !py-1 !min-h-0 text-xs !text-danger hover:!bg-red-50"><Trash2 size={12} /></button>
                            </div>
                        </motion.div>
                    ))
                }
